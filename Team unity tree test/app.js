...
function ensureProjectAccess() {
  ...
  try {
    const currentUserName = localStorage.getItem('team-unity-current-user') || '';
    if (currentUserName) {
...
function visualizeMetaSprites(selector, image, metaFrames) {
  ...
  }, '#meta-detection-info', `Found: ${metaFrames.length} frames in .meta file`);
}
...
function visualizeXmlSprites(selector, image, xmlSprites) {
  ...
  }, '#xml-detection-info', `Found: ${xmlSprites.length} sprites in XML`);
}
...
function previewXmlEyeLidSplit(selector, image, atlasEntries) {
  ...
  createOverlayPreview(selector, image, (ctx, width, height) => {
    ...
    eyeInfo.matches.forEach((sprite) => {
      const tint = sprite.splitSide === 'left' ? 'rgba(84, 160, 255, 0.9)' : 'rgba(255, 132, 100, 0.9)';
  ...
  }, '#xml-detection-info', `Eye/lid split preview: ${eyeInfo.matches.length} valid entries (${eyeInfo.left.length} left / ${eyeInfo.right.length} right)`);
}
...
function initializeMetaCutterPage() {
  ...
  const refreshMetaPreview = async () => {
    ...
    try {
      ...
      previewMetaText(firstMeta, text, splitEyeSpritesEnabled, imageFiles[0]);
  ...
  function previewMetaText(file, text, splitEyeSpritesEnabled = false, imageFile = null) {
    ...
    let summary = [
      ...
      `Sprite frames found: ${spriteFrames.length}`,
    ];
    ...
    if (spriteFrames.length > 0) {
      ...
      if (text.includes('Sprites:')) {
        summary.push('');
        summary.push('⚠️ Sprites: section found but could not parse frames.');
        summary.push('Check the format of sprite definitions.');
      } else {
        summary.push('');
        summary.push('⚠️ No Sprites: section found in file.');
        summary.push('Will use auto-detection instead.');
      }
    }

    const imageFiles = Array.from(imageInput.files || []);
    if (spriteFrames.length > 0 && imageFiles.length > 0 && imageFile) {
      readFileAsDataUrl(imageFile).then(async (dataUrl) => {
        const image = await loadImageFromDataUrl(dataUrl);
        const normalizedFrames = spriteFrames.map((entry) => ({
          ...entry,
          y: image.height - entry.y - entry.height,
        ...
        const preview = document.querySelector('#meta-preview');
        if (preview) {
          preview.innerHTML = `<div style="padding: 18px; font-family: monospace; white-space: pre-wrap; line-height: 1.6; color: var(--text);">${summary.join('\n')}</div>`;
        }
      });
    } else {
      const preview = document.querySelector('#meta-preview');
    ...
    if (spriteFrames.length > 0) {
      setStatus(`Found ${spriteFrames.length} sprites in ${file.name}.`, 'success');
    } else {
      setStatus(`Loaded ${file.name} (no sprite frames detected - will auto-detect).`, 'info');
    }
  }
